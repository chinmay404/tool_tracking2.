...
class ProductIndex(models.Model):
    ...
    is_printed_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL,
                                      null=True, editable=False, related_name='printed_product_indexes')
    is_complete = models.BooleanField(default=False)
    gate_inward_No = models.CharField(
        max_length=10, null=True)      # Gate Inward No
    products = models.ManyToManyField(
        Product, through='ProductIndexItem')  # Material Name
    ...
    received_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL,
                                    null=True, editable=False, related_name='received_product_indexes')
    arrive_date = models.DateTimeField(
        default=timezone.now)  # Gate Inward date
...
class ProductIndexItem(models.Model):
    ...
    short_quantity = models.FloatField(null=True, default=0.0)
...
class Master(models.Model):
    ...
    Material_weight = models.FloatField(default=0.0)
    is_insert = models.BooleanField(default=False)
    ...
    machine_added_date = models.DateTimeField(
        default=None, null=True, blank=True)
    machine_remove_date = models.DateTimeField(
        default=None, null=True, blank=True)
    status = models.CharField(
    ...
    balancing_report = models.FileField(
        ...
        validators=[
            FileExtensionValidator(allowed_extensions=[
                                   'pdf', 'jpg', 'jpeg', 'png', 'gif', 'csv', 'xls', 'xlsx']),
